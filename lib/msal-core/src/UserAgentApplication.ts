...
export class UserAgentApplication {
  ...
  private handleAuthenticationResponse(hash: string): void {
    ...
    let self: UserAgentApplication = null;
    let isPopup: boolean = false;
    let isWindowOpenerMsal = false;

    // Check if the current window opened the iFrame/popup
    try {
      isWindowOpenerMsal = !!(window.openedWindows.find(wndow => wndow.location.hash === hash));
    } catch (err) {
    ...
    if (isWindowOpenerMsal) {
      self = window.msal as UserAgentApplication;
      isPopup = true;
    } else if (window.parent && window.parent.msal) {
      self = window.parent.msal as UserAgentApplication;
    }
